...
import com.freezer.android_course_week_6.ActivityRequestCode
...
class NowPlayingFilmListFragment : Fragment() {
    ...
    suspend fun getDataFromApi(page: Int) {
        if(page == 1) {
            films = TMDBService.getInstance().getApi().getNowPlaying(page = 1).results
        }
        else {
            films.addAll(TMDBService.getInstance().getApi().getNowPlaying(page = page).results)
        }
    }
}
